...
simplePLS <- function(obsData, smMatrix, mmMatrix, inner_weights = path_weighting, maxIt=300, stopCriterion=7, measurement_mode_scheme){
  ...
  normData <- corpcor::wt.scale(x = obsData[, mmVariables], center = TRUE, scale = TRUE
                                ,w = weight_vector
                                )
  ...
  for (iterations in 0:maxIt)  {
    ...
    construct_scores <- corpcor::wt.scale(x = construct_scores,center = TRUE,scale = TRUE
                                          , w = weight_vector
                                          )
  ...
  rSquared <- calc_insample(obsData, construct_scores, smMatrix, dependant, stats::cov.wt(construct_scores, wt = weight_vector, cor = TRUE)$cor)
                                            #weights::wtd.cor(construct_scores,weight = weight_vector)$correlation)
