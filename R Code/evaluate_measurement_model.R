...
validity <- function(seminr_model) {
  list(
    ...
    antecedent_vifs = antecedent_vifs(seminr_model$smMatrix, stats::cov.wt(seminr_model$construct_scores, wt = weight_vector, cor = TRUE)$cor),
    fl_criteria = fl_criteria_table(seminr_model)
  )
}

cross_loadings <- function(seminr_model) {
  ret <- stats::cov.wt(cbind(seminr_model$data[, seminr_model$mmVariables], seminr_model$construct_scores), cor = TRUE, wt = weight_vector)$cor[colnames(seminr_model$data[, seminr_model$mmVariables]), colnames(seminr_model$construct_scores)]
