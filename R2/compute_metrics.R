...
return_AIC_BIC <- function(i, seminr_model) {
  ...
  rsq <- cor_rsq(stats::cov.wt(seminr_model$construct_scores, wt = weight_vector, cor = TRUE)$cor, dv_name = i, iv_names = antecedents)
...
compute_construct_rhoA <- function(weights, mmMatrix, construct, obsData) {
  ...
  indicators <- corpcor::wt.scale(x = obsData[,mmMatrix[mmMatrix[,"construct"]==construct, "measurement"]], w = weight_vector, center = TRUE, scale = TRUE)
  S <- cov.wt(indicators, wt = weight_vector)$cov
