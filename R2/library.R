...
path_weighting <- function(smMatrix, construct_scores, dependant, paths_matrix) {
  ...
  #inner_paths <- weights::wtd.cor(construct_scores,construct_scores, weight = weight_vector)[[1]] * t(paths_matrix)
  inner_paths <- stats::cov.wt(construct_scores, wt= weight_vector, cor = TRUE)$cor * t(paths_matrix)
  ...
  for (i in 1:length(dependant))  {
    ...
    #independent_2 <- weights::wtd.cor(construct_scores[,independant, drop = FALSE], weight = weight_vector)
    
    independent_2 <- stats::cov.wt(construct_scores[,independant, drop = FALSE], cor = TRUE, wt = weight_vector)$cor

    #dependant_2 <- weights::wtd.cor(construct_scores[,independant],  construct_scores[,dependant[i]], weight = weight_vector)

    matrix_independant <- as.matrix(construct_scores[,independant])

    colnames(matrix_independant) <- independant

    matrix_dependent <- as.matrix(construct_scores[,dependant[i]])

    colnames(matrix_dependent) <- dependant[i]

    dependant_2 <- cov.wt(cbind(matrix_dependent, matrix_independant), wt = weight_vector, cor = TRUE)$cor[independant,1]

     # if(class(independent_2)[[1]] == "list") {
      #  independent_2 <- independent_2[[1]]
      #} else if(class(independent_2)[[1]] == "matrix") {
      #  independent_2 <- independent_2[,1]
      #}


      #if(class(dependant_2)[[1]] == "list") {
      #  dependant_2 <- dependant_2[[1]]
      #} else if(class(dependant_2)[[1]] == "matrix") {
      #  dependant_2 <- dependant_2[,1]
      #}

    inner_paths[independant,dependant[i]] = solve(independent_2, dependant_2)
...
calculate_loadings <- function(weights_matrix,construct_scores, normData) {
  ...
  return(as.matrix((stats::cov.wt(cbind(normData,construct_scores), wt = weight_vector)$cov[colnames(normData), colnames(construct_scores)]) * weights_matrix))
...
estimate_path_coef <- function(smMatrix, construct_scores,dependant, paths_matrix) {
  ...
  for (i in 1:length(dependant))  {
    ...
    #independent_2 <- weights::wtd.cor(construct_scores[,independant, drop = FALSE]
                                      #, weight = weight_vector)

    #dependant_2 <- weights::wtd.cor(construct_scores[,independant],  construct_scores[,dependant[i]]
                                    #, weight = weight_vector)
  ...
  paths_matrix[independant,dependant[i]] = solve(independent_2, dependant_2)
...
standardize_outer_weights <- function(normData, mmVariables, outer_weights) {
  ...
  std_devs <- attr(corpcor::wt.scale((normData[,mmVariables]%*%outer_weights), center = FALSE, w = weight_vector),"scaled:scale")
...
mode_A  <- function(mmMatrix, i, normData, construct_scores) {
  ...
  var_1 <- as.matrix(normData[,mmMatrix[mmMatrix[,"construct"]==i,"measurement"]])

  colnames(var_1) <- mmMatrix[mmMatrix[,"construct"]==i,"measurement"]
  
  var_2 <- as.matrix(construct_scores[,i])
  
  colnames(var_2) <- i

  cov_weighted <- stats::cov.wt(x = cbind(var_1,var_2), wt = weight_vector)

  cov_weighted_2 <- matrix(cov_weighted$cov[colnames(var_1), colnames(var_2)])

  rownames(cov_weighted_2) <- colnames(var_1)

  return(cov_weighted_2)
}
#' @export
